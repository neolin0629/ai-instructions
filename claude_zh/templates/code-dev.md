...
- MRE 入口：有可运行逻辑的文件以 `if __name__ == "__main__":` 结尾，带真实感的 dummy data 作为最小可复现示例。纯库模块（`__init__.py`、纯类型/接口/常量模块）不需要
...
3. 循环无法避免时用 `numba.jit(nopython=True, cache=True)`
4. 大数据集评估峰值内存，必要时分块
...
- 网络请求、API、文件 IO 必须 try-except 并记录错误；多任务并行时单个任务失败不得让主线程崩溃
- 禁止裸 `except:` 和 `except Exception: pass`

## 测试与依赖

- 核心数值计算和纯函数 pytest 覆盖率 > 80%
