...
| 层 | 判据 | 举例 |
|---|---|---|
| **全局**（`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`） | 跨项目、猜不到，且**在还没有代码可模仿的新项目里依然成立** | 语言约定、存储分层选型、领域正确性护栏 |
| **单个仓库**（该仓库自己的指令文件） | 关于这个仓库的事实 | 领域、实际表结构、连接方式、该仓库的验证命令、项目编码规范 |
| **哪儿都不写** | 仓库自己会说 | 工具链与 lint 配置（`uv.lock`、`pyproject.toml`）、现有代码风格、测试命令 |
...
长期有效的跨项目默认值：语言、变更边界、新项目默认、存储分层、领域护栏、subagent 策略、产出物。这个文件在每个 session 里常驻，是唯一体积真正要紧的地方 —— 务必短。

### 2. `config.toml`（运行时配置，仅 Codex）

模型、推理强度、personality、子 Agent 并发数和 memories 的可移植默认值。
